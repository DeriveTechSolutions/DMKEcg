...
//----------------------------RTOR-----------------------------------
        ...
        rtor =((rtor >>2) & 0x3fff) ;
        float hr =  60 /((float)rtor*0.007789);
         int HR = (unsigned int)hr;  // type cast to int

        unsigned int RR = (unsigned int)rtor*8 ;  //8ms

        if (HR==179 || HR==183 && RR==336 || RR==344) // Fluctations to be avoided for higher Heart rate 180
        {
            HR = 180;                     // Send this approximate value for heart rate
            RR = 340;                     // Send this approximate value for R-R ratio
        }
        else if (HR==197 || HR==202 && RR==304 || RR==312)// Fluctations to be avoided for higher Heart rate 200
        {
            HR = 200;                     // Send this approximate value for heart rate
            RR = 308;                     // Send this approximate value for R-R ratio
        }
        else
        {
            HR = HR;                      // Otherwise send the original HR and RR
            RR = RR;
        }
        HR = HR;                          // Default send the original HR and RR
        RR = RR;

        F2A(SPI_temp_32b[1],res,0);       // Convert Decimal data to ASCII
        uart_string(res);                 // Send ASCII data on uart
        uart_string(" ");                 // Send blank-space on uart to differentiate between ECG data and Heart Rate
        F2A(HR,res1,0);                   // Convert Decimal data to ASCII
        uart_string(res1);                // Send ASCII data on uart
        uart_string(" ");                 // Send blank-space on uart to differentiate between previous data and next data


        __bis_SR_register(GIE);           //Enable Global Interrups
        __no_operation();
    }
}
